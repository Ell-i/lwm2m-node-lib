...
/**
 * Load the tables of pre-configured devices.
 */
function loadDevices(serverInfo, registry, config) {
    logger.info(context, 'Loading devices');

    if (config.devices) {
       for (var i in config.devices) {
           // Store the device, cf. lib/services/server/storeDevice
           var device = {
               name:     config.devices[i].name,
               address:  config.devices[i].address,
               port:     config.devices[i].port,
               path:     config.devices[i].path,
               type:     config.devices[i].type,
               lifetime: "85671", // Where does this come from?
               creationDate: new Date()
           };

           var endpoint = device.name,
               lifetime = "85671",
               version = undefined,
               binding = undefined,
               payload = '',
               callback = function(error, result) { /*XXX */};

           async.series([
               // async.apply(storeDevice, queryParams, req),
               apply(registry.register, device),
               ...
               apply(serverInfo.handlers['registration'].user, endpoint, lifetime, version, binding, payload)
           ], function(error, results) {
               if (error) {
                   logger.debug(context, 'An error was encountered registering device: ' + JSON.stringify(error));
               }
           });
       }
    }
...
function exportLoadDevices(serverInfo, config) {
    loadDevices(serverInfo, registry, config);
}
...
exports.loadDevices = exportLoadDevices;
